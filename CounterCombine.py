...
class FileWriter:
    ...
    def wrap_it_up(self, is_minutely):
        ...
        for counter_name in self.counter_cols:
            ...
            while row < get_no_days_in_year(self.year)*get_values_for_day(is_minutely):
    ...
    def __init__(self, year, is_minutely):
        ...
        self.worksheet = init_worksheet(self.worksheet, year, is_minutely)
...
def replace_null_values(counter_cols, year, is_minutely):
    ...
    worksheet = init_worksheet(worksheet, year, is_minutely)
    ...
    for counter_name in counter_cols:
        ...
        for row in range(0, get_no_days_in_year(year)*get_values_for_day(is_minutely)):
            ...
            if value == -1:
                ...
                worksheet.write(row, counter_cols[counter_name],
                                handle_null_value(data, row, counter_cols, counter_name,
                                    null_values, coords, is_minutely))
            elif value is not None:
                worksheet.write(row, counter_cols[counter_name], value)
...
def init_worksheet(worksheet, year, is_minutely):
...
def handle_null_value(data, row, counter_cols, counter_name, null_values, coords, is_minutely):
    ...
    if row > 0 and not null_values[counter_name][row-1][1] and not is_next_value_null(data, row, counter_cols[counter_name]):
        return data.iloc[row-1, counter_cols[counter_name]]
    else:
        value_from_prev_weeks = get_values_from_prev_weeks(data, row, counter_cols[counter_name], null_values[counter_name], is_minutely)
...
def combine_lanes_from_year(year, is_minutely):
    ...
    replace_null_values(cc, year, is_minutely)
