...
def get_x_train_basic(data, x_train_start_ix, var_len):
    return data.iloc[x_train_start_ix:x_train_start_ix + var_len, 2:].values.flatten()
...
def get_feature_values_for_fixed_time_period(data, pred_date, var_len, x_train_start_ix):
    x_train_basic = get_x_train_basic(data, x_train_start_ix, var_len)
    return np.concatenate((x_train_basic,
                                    [get_work_day_value(pred_date), pred_date.day,
                                     pred_date.month, pred_date.year]))
...
def get_start_row(date, is_minutely):
    return (date.timetuple().tm_yday - 1) * get_values_for_day(is_minutely)  # + hour


def get_feature_values(data, counter_name, is_minutely, pred_date, var_len, train_len):
    pred_date_ix = get_start_row(pred_date, is_minutely)
    y_train_start_ix = pred_date_ix - train_len
    x_train_start_ix = y_train_start_ix - var_len
    ...
    while y_train_start_ix < pred_date_ix:
        ...
        x_train.append(get_feature_values_for_fixed_time_period(data, pred_date, var_len, x_train_start_ix).tolist())
...
#   var_len - number of days before prediction day that will be taken into account when predicting
...
def prepare_data(data, counter_name, is_minutely, pred_date, var_len, train_len):

    x_train, y_train = get_feature_values(data, counter_name, is_minutely, pred_date, var_len, train_len)
    x_test = get_feature_values_for_fixed_time_period(data, pred_date, var_len, get_start_row(pred_date, is_minutely))\
        .reshape(1, -1).tolist()
...
def lasso_regression(data, is_minutely, pred_minute, pred_hour, pred_day, pred_month, year, var_len, train_len):
...
a = all_counter_names
lasso_regression(data, is_minutely, minute, hour, day, month, year, var_len, train_len)
